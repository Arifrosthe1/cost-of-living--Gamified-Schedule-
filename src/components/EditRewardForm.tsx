...
import type { Reward } from '../store/db';

export function EditRewardForm({ reward, onClose }: { reward: Reward; onClose: () => void }) {
    const { updateReward } = useEconomy();
    const [name, setName] = useState(reward.name);
    const [cost, setCost] = useState(reward.cost.toString());
    const [tier, setTier] = useState<'common' | 'rare' | 'epic'>(reward.tier);
