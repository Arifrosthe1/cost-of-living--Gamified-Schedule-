...
export function EditActionForm({ action, onClose }: { action: UserAction, onClose: () => void }) {
    const { updateCustomAction } = useEconomy();
        ...
        <div className="fixed inset-0 bg-white/80 backdrop-blur-md z-50 flex items-center justify-center p-4 transition-all animate-in fade-in duration-300">
            <div className="bg-white rounded-3xl shadow-2xl shadow-neutral-200/50 w-full max-w-md p-6 border border-neutral-100">
                <div className="flex items-center justify-between mb-8">
                    <h3 className="text-xl font-medium text-neutral-800">Edit Habit</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <button
                        type="submit"
                        className="w-full bg-neutral-900 text-white rounded-xl py-3.5 font-medium hover:bg-neutral-800 transition-colors active:scale-[0.98] flex justify-center items-center gap-2"
                    >
                        <Save size={18} />
                        Save Changes
                    </button>
